...
class engineer_feature(object):
    def __init__(self, dataframe,index_variable=None,label=None,include_variables=None,exclude_variables=None,date_variable=None):
    ...
    def box_plot_categorical_label(self):
        ...
        for y in self.categorical_variables:
                ...
                ax = sns.boxplot(x=y, y=self.label, data=self.dataframe)

                plt.title('Box plot of '+ y,size=16, y=1.05 )
                medians = self.dataframe.groupby([y])[self.label].median().values
